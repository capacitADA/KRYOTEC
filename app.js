...
<table style="margin-bottom: 4px;"><tr><td><strong>ESTADO:</strong> ${estado === 'Funcionando' ? '☒' : '☐'} Funcionando | ${estado === 'Fuera de servicio' ? '☒' : '☐'} Fuera de servicio | ${estado === 'Dar de baja' ? '☒' : '☐'} Dar de baja</td></tr></table>
...
<table><tr><td style="width: 25%;"><strong>Firma del Proveedor:</strong></td><td style="width: 25%;" class="firma-meddon">${sesionActual?.nombre || ''}</td><td style="width: 25%;"><strong>Firma (D1 SAS):</strong></td><td style="width: 25%;"><canvas id="firmaCanvas" width="100" height="40" style="width:100px;height:40px;border:1px solid #ccc;"></canvas></td></tr><tr><td><strong>Nombre Completo:</strong></td><td>${sesionActual?.nombre || ''}</td><td><strong>Nombre Completo:</strong></td><td>${funcNombre}</td></tr><tr><td><strong>Identificacion:</strong></td><td>${sesionActual?.cedula || ''}</td><td><strong>Identificacion:</strong></td><td>${funcId}</td></tr><tr><td colspan="2"></td><td colspan="2"><div style="text-align:center;">${selloUrl ? `<img src="${selloUrl}" style="max-height: 40px;">` : ''}</div></td></tr></table>
...
<script>setTimeout(() => { const canvas = document.getElementById('firmaCanvas'); if (canvas && d1FirmaDataUrl) { const ctx = canvas.getContext('2d'); const img = new Image(); img.onload = () => { ctx.drawImage(img, 0, 0, canvas.width, canvas.height); }; img.src = d1FirmaDataUrl; } }, 100);</script></body></html>`;
}
